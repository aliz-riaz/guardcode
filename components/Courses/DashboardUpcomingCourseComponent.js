...
function DashboardUpcomingCourseComponent(props) {
  useEffect(() => {
    if (props.switchValue == props.rightText) {
      props.dashboardUpcomingCourses(
        props.user_token,
      ...
      props.dashboardUpcomingCourses(props.user_token, [], "self");
    }
  }, [props.switchValue, props.seletedTeamMembers]);

  const CoursesList = () => {
    let Courses = <> </>;
    if (!_.isEmpty(props.dashborad_upcoming_courses)) {
      Courses = props.dashborad_upcoming_courses.map((course, key) => {
        let status_class = "";
        ...
        <div className="table-wrap">
          <div className="table-responsive courses-table">
            ...
            {!props.is_request_loader ? (
              !_.isEmpty(props.dashborad_upcoming_courses) ? (
                <div className="view_all">
                  <Link href="/upcoming-course">View all</Link>
                </div>
              ) : null
            ) : null}
          </div>
...
const mapDispatchToProps = (dispatch) => ({
  dashboardUpcomingCourses: (user_token, members, listType) =>
    dispatch(setDashboardUpcomingCourses(user_token, members, listType)),
});
