...
const UsersCardList = ({
  ...
  useEffect(() => {
    if (cardsData[0].is_default) {
      setSelectedCard({
        cardBrand: cardsData[0].card.brand,
        cardLast4: cardsData[0].card.last4,
        paymentMethod: cardsData[0].id,
      });
      setFieldValue("selectedPaymentMethod", cardsData[0].id);
      setFieldValue("selectedCardBrand", cardsData[0].card.brand);
      setFieldValue("selectedCardLast4", cardsData[0].card.last4);
    }
  }, []);
  return (
    <>
        ...
        >
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setFieldValue("selectedCardBrand", method.card.brand);
              setFieldValue("selectedCardLast4", method.card.last4);
            }}
          />
          <>
            {/* {method.card.brand} */}
