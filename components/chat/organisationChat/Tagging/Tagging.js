...
const Tagging = (props) => {
  ...
  // props.tags.tag.hasOwnProperty("name") != "" ? true : false
  // Object.keys(props.tagging).length === 0 ? true : false
  // props.tagging?.length == undefined ? true : false

  useEffect(() => {
    setTagAssign(props.tagging?.length == undefined ? true : false);
  }, [props.tagging]);
  ...
  const toggleTagingDropdown = () => {
    ...
    setTagAction("list");
    tagingDropdownOpen &&
      Object.keys(props.tagging).length !== 0 &&
      setTagAssign(true);
  };
  ...
  return (
    <>
      <div
        className={`${styles.dropdown_for_taging} d-flex justify-content-start`}
      >
        {/* {props.tagging?.length == undefined ? ( */}
        {tagAssign ? (
          <>
            {props.tags ? (
              <OverlayTrigger
              ...
              >
                <span
                  className={`${styles.applied_tag_box} position-relative `}
                >
                  <img
