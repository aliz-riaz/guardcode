...
function NextBackButton(props) {
  ...
  const onClickConfirmJobPost = async () => {
  ...
  const [isPostWithBoostLoading, setIsPostWithBoostLoading] = useState(false);
  ...
  return (
    <>
      <div className="container">
        <Row className="justify-content-center align-items-center row w-100">
          <Col className="col-12 col-md-4 text-left">
                  ...
                  <Button
                    onClick={() => {
                      if (
                        ...
                        // props.setLatestJobId(null)
                        props.setScreenToShowOnStaffing("jobs");
                        // props.setDiscardLinkForModalJobPost("/staffing");
                        props.setDiscardLinkForModalJobPost("/jobpost");
                      } else {
            ...
            {/* <Button onClick={() => props.goToBack()} disabled={isBrowser ? props.active_step<=1 : props.jobPostActiveStep<=1} className="btn w-100 btn_back w-md-auto btn-sm btn-gray w-100">BACK</Button> */}
          </Col>
          <Col className="col-12 col-md-4">
            {props.active_step >= 5 ? (
              <>
                ...
                {/* <a href={process.env.APP_URL+"/jobpost/preview"} target="_blank" className="align-items-center flex-nowrap mr-2 text-dark ml-2 d-none d-md-flex"> */}
                {/* <span className="pt-2 ml-2 cursor-pointer d-none d-md-inline-block" onClick={() => {
                  props.setShowJobPreview(true)}}>
                  <img src={process.env.APP_URL+"/images/eye.svg"} />
                  <span className="font-weight-bold fs-6 ml-2">Preview job</span>
                </span> */}
                {/* </a>  */}
              </>
            ) : (
              <div className={styles.button_wrapper}>
                {props.active_step != 4 && (
                  ...
                  >
                    {/* {props.avalible_connects > 0 && props.active_step == 4 */}
                    {/* {props.active_step == 4
                      ? "Confirm"
                      : props.active_step != 4
                      ? "Next"
                    ...
                    {/* {props.active_step == 4 && props.isSubmitting && (
                      <span className="ml-2">
                        <Spinner size={"sm"} color={"dark"} />
                      </span>
                    )} */}
                ...
                {props.active_step == 4 && (
                  <>
                    ...
                    <span
                      className="pt-2 ml-2 cursor-pointer d-none d-md-inline-block"
                      onClick={() => {
                        props.setShowJobPreview(true);
                      }}
                    >
                      <img src={process.env.APP_URL + "/images/eye.svg"} />
                      <span className="font-weight-bold fs-6 ml-2">
                        Preview job
                      </span>
                    </span>
                  </>
            ...
            {/* For later use Preview job button */}
      ...
      {/* <ModalContainer modalState={props.boostConfirmationModal}> // remove in new billing
